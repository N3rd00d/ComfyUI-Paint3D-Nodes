...
from ..utils import inpaint_atlas, save_tensor_image
...
class TexturedMeshModel(nn.Module):
    ...
    def forward_texturing(self, view_target, theta, phi, radius, save_result_dir, view_id=None, verbose=False):
        ...
        uncolored_mask_render = torch.from_numpy(
            cv2.erode(uncolored_mask_render[0, 0].detach().cpu().numpy(),
                      np.ones((erode_size, erode_size), np.uint8))).to(
    ...
    def texture_postprocess(self):
        ...
        self.texture_img_postprocess = nn.Parameter(
            torch.from_numpy(texture_img_npy_inpaint / 255.0).unsqueeze(0).permute(0, 3, 1, 2))
